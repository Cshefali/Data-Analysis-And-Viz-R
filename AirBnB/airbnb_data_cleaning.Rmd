...
Each of the categorical columns will be encoded with 1 and 0s.  

#### 1.1 Host Identity Verification  


```{r}
data4 <- data3 %>% 
          mutate(status_verified = ifelse(host_identity_verified == "unconfirmed" | is.na(host_identity_verified), 0, 1),
                 status_unconfirmed = ifelse(host_identity_verified == "verified" | is.na(host_identity_verified), 0, 1))
...
```{r}
# data4 <- data4 %>% 
#           mutate(cancel_strict = ifelse(cancellation_policy))
```
