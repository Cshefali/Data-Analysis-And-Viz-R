...
2. **argument 1 'a':**  
    ...
    - Example:  
        - **`scale_y_continuous(limits = (0,6), expand = c(2,0))`**  
            - new_lower_y_limit = 0 - (6-0)*2 = -12.  
            - new_upper_y_limit = 6 + (6-0)*2 = 18. 
            So, instead of limits being (0,6), now limits will be (-12,18) on Y axis and space will be added at both ends.  
            
3. **argument 2 'b':**  
    - additive factor.  
    - Only adds this specified unit to the given multiplicative factor.  
    - Formula:
        - new_lower_y_limit = lower_limit - (range of Y)*a - b  
        - new_upper_y_limit = upper_limit + (range of Y)*a +b  
        
## Using different values for `expand`:
...
### Modify the 2nd argument (additive factor)  
...
### Modify the first argument (multiplicative factor)
...
**Drawback of this method**- space gets added or removed from both ends of axis.  
...
### expand = expansion(mult = c(a,b), add = (m,n))
...
Below, keeping **`expand = expansion(mult = c(0,0.05))`** implies no space below '0' mark on Y axis and add 5% space above '22,000' mark. 
...
## Using expansion() on the discrete X axis: 
